...
fn main() {
    ...
    if !lgbm_root.exists() {
        let status = if target.contains("windows") {
            ...
            Command::new("cp")
            .args(&["-r", "deps", out_dir.as_str()])
            .status()
            .unwrap_or_else(|e| {
                panic!("Failed to copy ./deps to {}: {}", out_dir, e);
            });
...
//        .expect("Unable to generate bindings");
 ...
 //       .expect("Couldn't write bindings.");
    ...
    println!("cargo::rustc-link-search={}", Path::new(&out_dir).join("deps").display());
    println!("cargo:rustc-link-search={}", out_path.join("lib").display());
    // println!("cargo:rustc-link-search=native={}", dst.display());
   println!("cargo:rustc-link-search=native={}", Path::new(&out_dir).join("deps").display());
